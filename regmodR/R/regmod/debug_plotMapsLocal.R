debug_plotMapsLocal <- function(raster){
  
  require(utils)
  
  delme <- gmap(x = projectExtent(raster, crs = CRS("+init=epsg:3857")), type = "hybrid")
  plot(delme)
  rprobGM <- projectRaster(from = raster, delme )
                  ...
                  lonlat = FALSE, ...) 
{
  ...
  gurl <- paste(gurl, "center=", ctr, "&zoom=", zoom, "&size=", 
                s, "&maptype=", type, "&format=gif&sensor=false&scale=", 
                scale, sep = "")
  if (!is.null(style)) {
    style <- gsub("\\|", "%7C", style)
    style <- gsub(" ", "", style)
