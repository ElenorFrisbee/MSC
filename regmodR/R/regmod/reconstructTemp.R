reconstruct <- function(idxvalues, indices_rekon, maps_sel_incolumns, region_repr_columns, latitude, longitude, cruMapMean, cruMapStd, a1){
  ...
  if(idxvalues[3] != 0){
    ...
    mregres <- maps_multiple_regression(allhkas, indices_rekon * a1)
    ...
    if(sum(coefficient)==0){
      reconstructed               <-   indices_rekon * a1
      print('indexfeld_mlr')
